#!/usr/bin/env python3
"""启动训练 Job：Full fine-tune，清洗后原图，500 张/种，5 epoch"""
...
JOB_NAME_PREFIX = "dinov2-full-finetune-500"

HYPERPARAMETERS = {
    "epochs": "5",
    "batch-size": "8",              # Full fine-tune 显存占用大，batch 减半
    "lr": "0.00001",                # Full fine-tune 用小 LR (1e-5)
    "weight-decay": "0.0001",
    "freeze-backbone": "0",         # 解冻 backbone
...
def package_and_upload():
    ...
    s3_key = "training-code/sourcedir-full-finetune.tar.gz"
...
def main():
    print("=" * 60)
    print("训练: Full Fine-tune + 清洗后原图 + 500张/种")
    print("=" * 60)
    print(f"注意: batch-size=8, lr=1e-5 (backbone 用 1e-7)")
    print(f"预计训练时间: 5-8 小时")
    print()
